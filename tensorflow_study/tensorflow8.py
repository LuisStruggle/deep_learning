...
Y_train = 1.2 * X_train + np.random.uniform(-1.0, 1.0,
                                            n_train_samples)  # 加一点随机扰动
...
with tf.name_scope('Inference'):
    W = tf.Variable(
        initial_value=tf.truncated_normal(shape=[1]), name='weight')
...
with tf.Session() as sess:
    ...
    summary_writer = tf.summary.FileWriter(
        logdir=summary_dir, graph=sess.graph)
    ...
    for i in range(201):
        ...
        _, summary, train_loss, W_pred, b_pred = sess.run(
            [optimizer, merge, loss, W, b], feed_dict={
                X: X_batch,
                Y: Y_batch
            })
        ...
        print('step:{}, losses:{}, test_loss:{}, w_pred:{}, b_pred:{}'.format(
            i, train_loss, test_loss, W_pred[0], b_pred[0]))
